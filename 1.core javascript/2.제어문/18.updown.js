...
var start = 1;
var end = 50;
var answer = Math.floor(Math.random()*(end - start + 1) + start);
...
while(true) {
  ...
  switch(level) {
    ...
    case LOW: 
          ...
          break;
    default:
          level = +prompt(`난이도를 잘못 설정하셨습니다. 1 ~ 3 중에서 다시 입력해주세요. (1: 상, 2: 중, 3: 하)`)
          continue;
  }
  break;
}
...
while(true) {
  // 입력
  var input = +prompt(`${start} ~ ${end} 사이의 무작위 숫자를 맞춰보세요. ${answer}`);
  ...
  alert(`기회가 ${chance}번 남았습니다.`);
  
  // 게임 종료 조건
  if(chance <= 0) {
    alert(`기회를 모두 소진했습니다. 정답은 ${answer} GAME OVER!!`);
    break;
  }
}
