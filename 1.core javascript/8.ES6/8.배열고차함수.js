...
//   console.log(`내 이름은 ${user.userName}고요~ ${user.address}에 살아요`);
// }

userList.forEach(user => {
  console.log(`제 이름은 ${user.userName}고요~ ${user.address}에 살아요`);
});
...
console.log(`filter결과.userName: ${filteredUser[0].userName}`);


// map: 배열에서 특정 데이터들을 추출해서 새로운 배열에 매핑

// filter => 10개의 데이터를 필터링하면 10개 이하로 남음 (배열이 줄어듬)
//          ex) 10명 중 서울 사는 애들 필터링! -> 3명만 남음
// map -=> 10개의 데이터를 매핑하면 항상 10개짜리 배열이 나옴
//          ex) 10명 회원정보 중 이름 정보만 10개 가져와
...
// 회원목록에서 회원 이름과 회원의 나이와 주소를 추출할건데
